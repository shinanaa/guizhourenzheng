...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      resetForm() {
        this.dialogFormVisible = false
        this.$refs.dialogForm.resetFields() // clearValidate 取消验证状态颜色  resetFields // 清空验证表单所有，包括颜色和内容
        this.form = {}
        this.require = []
        // this.majorList = []
      },
      ...
      getTableData(urlName) {
        this.$http.getRequest(urlName).then(res => {
          if (res.code === 1) {
            ...
            this.lineData = res.resultList[0]
            this.loading = false
            this.getRequireData()
            this.drawLine()
          } else {
      ...
      getRequireData(datas) {
        ...
        if (datas) {
          ...
          this.lineData = this.tableList[0]

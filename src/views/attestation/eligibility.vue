  ...
  export default {
    ...
    methods: {
      ...
      handleCurrentRow(val) {
        this.require = []
        this.currentRow = val
        this.currentSchool = val.schoolYear
        this.currentMajor = val.major
        for (const i in val) {
      ...
      getTableData(urlName) {
        ...
        this.$http.getRequest(urlName).then(res => {
          if (res.code === 1) {
            ...
            that.tableList = res.resultList
            that.total = res.resultList.length
            this.currentSchool = res.resultList[0].schoolYear
            this.currentMajor = res.resultList[0].major
            that.loading = false
          } else {
            that.emptyText = '暂无数据'
          }
